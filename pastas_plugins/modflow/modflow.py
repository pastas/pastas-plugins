...
class ModflowDrn(ModflowPkgLoggerMixin):
    ...
    def parameter_names(self):
        ...
        -------
        list of str
            ``["DRN_drnHfrac", "DRN_drnC"]`` — elevation fraction and
            conductance.
        """
        return [self._name + "_drnHfrac", self._name + "_drnC"]
    ...
    def update_package(
        ...
        modflow_gwf : flopy.mf6.ModflowGwf
            Active flopy GWF model object.
        drnHfrac : float
            Drain elevation as fraction of aquifer thickness from bottom.
        drnC : float
    ...
    def update_parameters(self, mf6, params) -> None:
        ...
        mf6 : modflowapi.ModflowApi
            Active MODFLOW 6 API instance.
        params : tuple of float
            ``(drnHfrac, drnC)`` — elevation fraction and conductance.
        """
        drnHfrac, drnC = params
        # CROSS-PACKAGE READ: drain elevation is a fraction of the aquifer
        # thickness, so it depends on the calibrated DIS/TOP and DIS/BOT.
        # ORDERING DEPENDENCY: requires ModflowDis.update_parameters() to have
        # run first so that DIS/TOP and DIS/BOT already hold calibrated values.
        ...
        drnH = botm + drnHfrac * (top - botm)
        self.api.init_ptrs(mf6)
        self.api.set_value_ptr("ELEV", np.array([drnH]))
        self.api.set_value_ptr("COND", np.array([drnC]))
...
class ModflowDrn2(ModflowPkgLoggerMixin):
    """DRN package wrapper. Calibrates drain elevation fraction and conductance."""
...
class ModflowSto2(ModflowPkgLoggerMixin):
    """Experimental STO variant combining confined SS and unconfined SY."""
    ...
    def parameter_names(self):
        ...
        -------
        list of str
            ``["STO_drnSfrac", "STO_drnS", "STO_S"]``.
        """
        return [self._name + "_drnSfrac", self._name + "_drnS", self._name + "_S"]
        ...
        parameters = DataFrame(
            {
                ...
                "vary": [True, True, True],
                "name": [name, name, name],
                "package": [self._name, self._name, self._name],
    ...
    def update_package(
        ...
        """Write STO package with confined SS and unconfined SY.
        ...
        modflow_gwf : flopy.mf6.ModflowGwf
            Active flopy GWF model object.
        drnSfrac : float
            Fraction of aquifer thickness defining the drain/SY transition.
        drnS : float
            Confined specific storage [1/L].
        S : float
    ...
    def update_parameters(self, mf6, params) -> None:
        """Write calibrated SS and SY to MODFLOW memory.
