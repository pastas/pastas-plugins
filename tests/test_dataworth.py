...
from pastas_plugins.dataworth import (
    DataWorth,
...
class TestObservationNoiseCovariance:
    ...
    def test_covariance_objfun_target_override(self, dataworth_instance):
        ...
        C_eps_noise = dataworth_instance.observation_noise_covariance(
            objfun_target="noise"
        )
        C_eps_residuals = dataworth_instance.observation_noise_covariance(
            objfun_target="residuals"
        )
...
class TestDataWorthPerObservation:
    ...
    def test_data_worth_per_observation_shape(self, dataworth_instance):
        ...
        n_obs = len(dataworth_instance.ml.observations())
        n_params_vary = len(
            dataworth_instance.ml.parameters[dataworth_instance.ml.parameters["vary"]]
        )

        assert df.shape[0] == n_obs
        assert (
            df.shape[1] == 1 + n_params_vary
        )  # 1 for overall + n_params_vary for per-parameter
    ...
    def test_data_worth_per_observation_columns(self, dataworth_instance):
        ...
        assert df.columns[0].startswith(
            "Δ"
        )  # Either "Δlogdet" or "Increase in gen. std. err. (%)"

        # Remaining columns should be for varying parameters
        varying_params = dataworth_instance.ml.parameters[
            dataworth_instance.ml.parameters["vary"]
        ].index
...
class TestRecomputeJacobian:
    ...
    def test_recompute_jacobian_basic(self, simple_pastas_model):
        ...
        new_observations = pd.Series(
            np.random.randn(10) + 10, index=new_obs_index, name="new_obs"
        )
    ...
    def test_recompute_jacobian_objfun_target(self, simple_pastas_model):
        ...
        # Test with residuals target using the calibrated model which has noise model
        # This requires a model with noise_alpha parameter
        # For now, just test that noise target works
        assert J_new_noise is not None
...
class TestDataWorthPerAddedObservation:
    ...
    def test_data_worth_per_added_observation_basic(self, simple_pastas_model):
        ...
        new_observations = pd.Series(
            np.random.randn(5) + 10, index=new_obs_index, name="new_obs"
        )
    ...
    def test_data_worth_per_added_observation_removes_duplicates(
        self, simple_pastas_model
    ):
        ...
        new_observations = pd.Series(
            np.random.randn(len(all_index)) + 10, index=all_index, name="new_obs"
        )
...
class TestEdgeCases:
    ...
    def test_observation_noise_covariance_invalid_objfun_target(
        self, dataworth_instance
    ):
